      ...
      if (time < this.configService.bot.salesLookbackSeconds) {
        ...
        try {
          sales.push({
            ...
            txHash: sale.txHash,
            cacheKey: cacheKey,
            permalink: `https://marketplace.forgottenrunes.com/${c.chain ??
              'ethereum'}/asset/${c.tokenContract}:${sale.token.tokenId}`,
            thumbnail,
        ...
        this._logger.warn(`Sale too old to broadcast: ${cacheKey} ${time}s`);
      }
    }
    return sales.reverse();
  }
      ...
      if (time < this.configService.bot.salesLookbackSeconds) {
        ...
        try {
          const formattedListing = {
            ...
            sellerName: sellerName ? `(${sellerName})` : ``,
            cacheKey,
            fmLink: `https://marketplace.forgottenrunes.com/${c.chain ??
              'ethereum'}/asset/${c.tokenContract}:${tokenId}`,
